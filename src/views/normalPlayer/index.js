...
const NormalPlayer = props => {
    const {currentTime,handlePlay,percent,exitFullScreen,handleShowPlayList,movingProgress,endChangeProgress,handlePrev,handleNext,playing,currentSong,duration,mode,changeMode,changeProgress} = props

    const getPlayMode = () => {
        if (mode===playMode.sequence){
            return <PlayCycle theme="outline" size="40" fill="#fff" strokeWidth={2} strokeLinejoin="bevel" strokeLinecap="butt"/>
        }else if(mode===playMode.loop){
           return <PlayOnce theme="outline" size="40" fill="#fff" strokeWidth={2} strokeLinejoin="bevel" strokeLinecap="butt"/>
        }else{
            return <ShuffleOne theme="outline" size="40" fill="#fff" strokeWidth={2} strokeLinejoin="bevel" strokeLinecap="butt"/>
        }
    }
                ...
                <div className="h-40 space-y-4">
                    ...
                    <div className="flex justify-around items-center">
                        ...
                        <button onClick={handlePlay}>
                            {
                                playing?
                                    <PauseOne theme="outline" size="50" fill="#fff" strokeWidth={2} />:
                        ...
                        <button onClick={handleShowPlayList}>
                            <MusicList theme="outline" size="40" fill="#fff" strokeWidth={2} />
                        </button>
                    </div>
                </div>
            </div>
