...
const ProgressBar = props => {
    const {currentTime,percent,duration,endChangeProgress,movingProgress} = props
    ...
    return (
        <div className="progressBarWrapper flex justify-between items-center space-x-2">
            <span className="text-white text-sm">{currentTime?formatTime(currentTime*1000):'00:00'}</span>
            <div
                ref={progressBar}
            ...
            <span className="text-sm text-white">{formatTime(duration*1000)}</span>
        </div>
    )
}
