...
const PlayList = props => {
  ...
  return (
    <div>
      {showConfirm ? (
        <Confirm closeConfirm={closeConfirm} clearAll={clearAll} />
      ) : null}
      <div
      ...
      >
            ...
            return (
              ...
              >
                <div className="truncate" onClick={() => clickToPlay(index)}>
                  {index === currentIndex ? (
                    <h3 className="text-red-600 align-middle inline-block text-lg max-w-3/4 truncate">
                      {item.name}{' '}
                    </h3>
                  ) : (
                    <h3 className="text-white align-middle inline-block text-lg max-w-3/4 truncate">
                      {item.name}{' '}
                    </h3>
                  )}
                  <span className="ml-2 inline align-middle truncate text-gray-400">
                    {item.ar ? getName(item.ar) : getName(item.artists)}
                  </span>
                </div>
                <button
                  className="flex-shrink-0 w-4"
                  onClick={e => clickToDel(e, item)}
                >
        ...
        <button
          ...
          className="flex-shrink-0 mx-auto h-8"
        >
